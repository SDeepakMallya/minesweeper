...
def gen_cell_objects(mine_space, rows, cols):
    ...
    for i in range(rows):
        cells.append([])
        for j in range(cols):
            elem = mine_space.find_element(By.ID, "%d_%d" %(i+1, j+1))
            cells[i].append(elem)
    return cells
...
def solver_time(game):
    ...
    """
    timer = 0
    timer_cell = ['ones', 'tens', 'hundreds']
    weights = [1, 10, 100]
    for t in range(3):
        w = game.find_element(By.ID, 'seconds_' + timer_cell[t])
...
def start_game(browser, mode='intermediate', attempt=0):
    ...
    face = mine_space.find_element(By.ID, 'face')
    cells = gen_cell_objects(mine_space, rows, cols)

    retry = 'y'
    while retry == 'y':
        attempt += 1
        ans = input("Proceed with guesses when required automatically?:(y/n) ")
        if ans == 'y':
            guess = True
        else:
            guess = False
